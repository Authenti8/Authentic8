...
export class TelemetryDelivery {
  ...
  constructor(private readonly client: AgentHttpClient, private readonly token: string,
    private readonly identity: EnrolledIdentity,
    private readonly persist: typeof saveIdentity = saveIdentity) {
    if (identity.pendingEvent) {
      identity.pendingEvents = [identity.pendingEvent, ...(identity.pendingEvents ?? [])];
      identity.pendingEvent = undefined;
    }
  }

  async enqueue(event: TelemetryEnvelope) {
    ...
    pending.push(event); this.identity.pendingEvents = pending;
    await this.persist(this.token, this.identity);
  }

  async enqueueClaimed(event: TelemetryEnvelope, claimId: string, nextIndex: number,
    chainState: EnrolledIdentity["chainState"]) {
    const pending = this.identity.pendingEvents ?? [];
    if (pending.length >= 1_000) throw new Error("Encrypted telemetry queue capacity exceeded.");
    pending.push(event); this.identity.pendingEvents = pending;
    this.identity.browserEvidenceClaim = { claimId, nextIndex };
    this.identity.chainState = chainState;
    await this.persist(this.token, this.identity);
  }

  async completeBrowserClaim() {
    this.identity.browserEvidenceClaim = undefined;
    await this.persist(this.token, this.identity);
  }
  ...
  async flush(force = false) {
    ...
    while (this.identity.pendingEvents?.length) {
      try {
        const event = this.identity.pendingEvents[0]!;
        await this.client.post("agent/telemetry", event);
        this.identity.pendingEvents.shift();
        if (event.eventType === "MONITORING_STARTED") this.identity.monitoringStarted = true;
        this.failures = 0; await this.persist(this.token, this.identity);
      } catch {
        this.failures += 1; this.retryAt = Date.now() + backoff(this.failures); return false;
      }
    }
    return true;
  }
