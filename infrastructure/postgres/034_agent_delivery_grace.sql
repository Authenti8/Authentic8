...
DECLARE session_row verification_sessions; device candidate_devices; prior telemetry_events;
  existing telemetry_events; supplied_sequence BIGINT := (input->>'sequenceNumber')::BIGINT;
  event_time TIMESTAMPTZ := (input->>'eventTimestamp')::TIMESTAMPTZ;
  event_deadline TIMESTAMPTZ; consumption JSONB;
BEGIN
  ...
  event_deadline := session_row.eligible_end;
  IF input->>'eventType' = 'MONITORING_STOPPED' THEN
    event_deadline := event_deadline + interval '5 minutes';
  END IF;
  ...
  IF session_row.id IS NULL OR device.id IS NULL
    ...
    OR (now() > session_row.eligible_end AND session_row.status <> 'MONITORING_ACTIVE')
    OR now() > session_row.eligible_end + interval '5 minutes' THEN
  ...
  END IF;
  IF event_time < session_row.eligible_start
    OR event_time > event_deadline
    OR event_time > now() + interval '5 minutes' THEN
  ...
  IF prior.id IS NULL AND (supplied_sequence <> 0
    OR NULLIF(input->>'previousEventHash', '') IS NOT NULL) THEN
  ...
  VALUES ((input->>'eventId')::UUID, session_row.id, supplied_sequence, input->>'eventType',
    event_time, (input->>'monotonicTimestamp')::BIGINT, input->>'platform', input->'payload',
    input->>'payloadHash', NULLIF(input->>'previousEventHash', ''), input->>'agentVersion',
...
REVOKE ALL ON FUNCTION authenti8_ingest_agent_event(JSONB) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION authenti8_ingest_agent_event(JSONB) TO service_role;

INSERT INTO schema_migrations(version) VALUES ('034_agent_delivery_grace')
